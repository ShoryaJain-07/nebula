...
const Header = () => {
  ...
  return (
    <>
      ...
      <div
        className={`md:h-[100px] h-[60px] mx-auto border-box w-full sticky top-0 z-[4]`}
      >
        <header
          className={`h-full max-w-[100%] transition-all duration-[.3s] delay-300 ${
            showHeaderBg && styles.header_glassBg
          } md:border-b-[1px] md:border-b-[#D2ECFF]  font-orbitron text-[#fff] bg-cover`}
        >
          <nav
            className={`flex items-center h-full w-[90%] mx-auto xl:justify-around justify-between`}
          >
            <aside className="flex items-center gap-[10px]">
              <Image
                src={nebulaLogo}
                alt="nebula-logo"
                className="lg:h-[40px] lg:w-[40px] h-[35px] w-[35px]"
              />
            ...
            <aside className="lg:flex hidden items-center gap-[30px]">
              <ul className="flex items-center gap-[60px]">
                <Link
                  className={`${styles.header_link} ${linkStyles} ${
                    path == "/stats" ? "bg-[#fff] text-[#100e0e]" : ""
                  }`}
                  href="/stats"
                >
                  {" "}
            ...
            <div className="flex lg:hidden items-center gap-[10px]">
              <UserRound
                ...
                className={`bg-[#fff] ${styles.avatar_bg_glassGradient} w-[35px] rounded-[50%] p-[5px] border-[1px] h-[35px] cursor-pointer `}
              />
              <Menu
      ...
      <MobileHeader
        showMobileMenu={setOpenMobileMenu}
