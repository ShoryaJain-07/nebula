...
const MobileHeader: FC<IProps> = ({ isMobile, showMobileMenu }) => {
    ...
    return (
      <div
        className={`fixed z-[10] font-orbtiron ${
          isMobile
            ? "opacity-[1] visible translate-x-[-0px]"
            : "opacity-[0] invisible translate-x-[100px]"
        } transition-all delay-150 duration-[1s] w-full top-0 right-0 h-screen ${
          styles.mobile_glassBg
        } text-[#fff]`}
      >
        <aside className="w-[60%] float-right h-full flex flex-col items-center p-[15px] bg-[#10121B]">
          ...
          <div className="flex flex-col items-start min-[250px] gap-[30px]">
            ...
            <Link onClick={closeHeader} href="#" className={`${linkStyles}`}>
              {" "}
              Mint
            </Link>
            <a href="#" target="_blank" className={`${linkStyles}`}>
              {" "}
              Layer-Zero Scan{" "}
            </a>
          </div>
